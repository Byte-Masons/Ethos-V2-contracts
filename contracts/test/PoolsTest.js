...
contract('ActivePool', async accounts => {
  ...
  let activePool, mockBorrowerOperations, collateralConfig, troveManager, mockRedemptionHelper, mockLiquidationHelper
  ...
  beforeEach(async () => {
    ...
    const collateral1 = await ERC20.new("Wrapped Ether", "wETH", 12, treasury.address, 0); // 12 decimal places
    const collateral2 = await ERC20.new("Wrapped Bitcoin", "wBTC", 8, treasury.address, 0); // 8 decimal places
  ...
  it('simulate profit, check that send collateral distributes profit', async () => {
    ...
    assert.equal((await collaterals[0].balanceOf(treasury.address)).toString(), '1000000000000000000') // 1 ether
  ...
  it('simulate profit and reduce yielding percentage, check that send collateral distributes profit', async () => {
    ...
    // check profit distribution
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(treasury.address)).toString(), '3000000000000000000') // 1 + 2 = 3 ether
    ...
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '2800000000000000000')
  })
  ...
  it('simulate profit and reduce yielding percentage, check that pull collateral distributes profit', async () => {
    ...
    // check profit distribution
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(treasury.address)).toString(), '1000000000000000000') // 1 ether
    ...
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '4550000000000000000')
    ...
    // check profit distribution
    assert.equal((await collaterals[0].balanceOf(treasury.address)).toString(), '3000000000000000000') // 1 + 2 = 3 ether
  ...
  it('simulate profit, check that manual rebalance distributes profit', async () => {
    ...
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '5000000000000000000')
    ...
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(activePool.address)), '6000000000000000000') // 6 ether
    assert.equal((await activePool.yieldingAmount(collaterals[0].address)), '4000000000000000000') // 4 ether
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '4000000000000000000')
    ...
    assert.equal((await activePool.yieldingAmount(collaterals[0].address)), '4500000000000000000') // 4.5 ether
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '4500000000000000000')
  })

  it('simulate profit, update treasury', async () => {
    await setReasonableDefaultStateForYielding();
    ...
    // update treasury
    const newTreasury = await NonPayable.new()
    try {
      // should fail if called by non-owner
      await activePool.updateTreasury(newTreasury.address, { from: alice })
    } catch (err) {
      assert.include(err.message, "revert")
    }
    await activePool.updateTreasury(newTreasury.address, { from: owner })
    ...
    // check profit distribution (old treasury still has 1 ether, new treasury has 2 ether)
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(treasury.address)).toString(), '1000000000000000000')
    assert.equal((await collaterals[0].balanceOf(newTreasury.address)).toString(), '2000000000000000000')
  ...
  it('simulate profit and increase yielding percentage, check that manual rebalance distributes profit', async () => {
    ...
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '6750000000000000000')
  })
  ...
  it('simulate profit and reduce yielding percentage, check that manual rebalance distributes profit', async () => {
    ...
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(activePool.address)), '6800000000000000000') // 6.8 ether
    assert.equal((await activePool.yieldingAmount(collaterals[0].address)), '3200000000000000000') // 3.2 ether
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '3200000000000000000')
    ...
    th.assertIsApproximatelyEqual((await collaterals[0].balanceOf(vaults[0].address)), '2250000000000000000')
  })
