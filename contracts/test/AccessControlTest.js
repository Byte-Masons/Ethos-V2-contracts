...
contract('Access Control: Liquity functions with the caller restricted to Liquity contract(s)', async accounts => {
  ...
  describe('ActivePool', async accounts => {
    // sendCollateral
    it("sendCollateral(): reverts when called by an account that is not BO nor TroveM nor SP nor LH", async () => {
      // Attempt call from alice
      try {
        ...
        assert.include(err.message, "revert")
        assert.include(err.message, "Caller is neither BO nor TroveM nor SP nor LH")
      }
    })
    ...
    // decreaseLUSD
    it("decreaseLUSDDebt(): reverts when called by an account that is not BO nor TroveM nor SP nor RH", async () => {
      // Attempt call from alice
      try {
        ...
        assert.include(err.message, "revert")
        assert.include(err.message, "Caller is neither BO nor TroveM nor SP nor RH")
      }
    })
  ...
  describe('StabilityPool', async accounts => {
    ...
    // offset
    it("offset(): reverts when called by an account that is not LiquidationHelper", async () => {
      // Attempt call from alice
      try {
        ...
        assert.include(err.message, "revert")
        assert.include(err.message, "Caller is not LiquidationHelper")
      }
    })
  })
  ...
  describe('LUSDToken', async accounts => {
    ...
    it("upgradeProtocol(): reverts when called by an account that is not Governance", async () => {
      ...
      newContracts.reapervaults = coreContracts.reapervaults;
      const LQTYContracts = await deploymentHelper.deployLQTYTesterContractsHardhat(multisig)
      await deploymentHelper.connectLQTYContracts(LQTYContracts)
